...
```{r}
# Developed
hold <- groupfdata %>% filter(Country.Code.x != Country.Code.y) %>%
...
```{r}
hold2 <- groupfdata %>% filter(Country.Code.x != Country.Code.y) %>%
...
If we normalize flights to a destination country by the population of said destination country, i.e. assume countries with higher populations should expect more flights we find different results:
...
hold %>% left_join(pops, by=c("Country.Code.y"="country")) %>% 
  mutate(n_count = as.integer(count)/as.integer(pop)) %>% 
  arrange(desc(n_count)) -> developed_dests

head(developed_dests,5)
```

We find instead that for developing countries, the most popular destinations (once normalized) are Antigua and Barbuda, Iceland, St. Kitts and Nevis, Bermuda, and Austria.

```{r}
developed_dests$top5<-c(rep(1,5),rep(0,144)) #Changed developing_dests to developed_dests
ggplot(developed_dests,aes(factor(Country.Code.y),n_count,col=factor(top5))) + 
  geom_point() +
  scale_y_log10() + 
...
cbind(head(developed_dests,5),places) -> temp
...
```{r}
hold2 %>% left_join(pops, by=c("Country.Code.y"="country")) %>% 
  mutate(n_count = as.integer(count)/as.integer(pop)) %>% 
  arrange(desc(n_count)) -> developing_dests

head(developing_dests,5)
```
...
cbind(head(developing_dests,5),places) -> temp
...
If we include travel within one's own country (`source.country` can equal `destination.country`), the results are exactly the same for developed countries:

```{r}
groupfdata %>% filter(developed.x == 1) %>%
  group_by(Country.Code.y) %>%
  summarize(count = n()) %>% 
  left_join(pops, by=c("Country.Code.y"="country")) %>% 
  mutate(n_count = as.integer(count)/as.integer(pop)) %>% 
  arrange(desc(n_count)) -> developed_dests2

head(developed_dests2,5)
```
...
```{r}
groupfdata %>% filter(developed.x == 0) %>%
  group_by(Country.Code.y) %>%
  summarize(count = n()) %>%
  arrange(desc(count)) %>% 
  left_join(pops, by=c("Country.Code.y"="country")) %>% 
  mutate(n_count = as.integer(count)/as.integer(pop)) %>% 
  arrange(desc(n_count)) -> developing_dests2
head(developing_dests2,5)
```
...
> I looked at travel that could occur within one's own country. All sig. 
> Look at both or not?

```{r}
Outbound <- groupfdata %>% group_by(developed.x) %>% summarize(total_flights = n())
total_DVG <- readr::parse_number(Outbound[[1,2]][1])
total_DVD <- readr::parse_number(Outbound[[2,2]][1])

hold3 <- groupfdata %>% filter(developed.x == 1) %>%
  group_by(destination.country) %>%
  summarize(count = n()) %>%
  arrange(desc(count))

# normal has normalized

```

```{r}
# For Developed (Source can Equal Destination):
p_UK <- readr::parse_integer(hold3[[1,2]][1])
p_Ger <- readr::parse_integer(hold3[[2,2]][1])
p_SP <- readr::parse_integer(hold3[[3,2]][1])
p_IT <- readr::parse_integer(hold3[[4,2]][1])
p_FR <- readr::parse_integer(hold3[[5,2]][1])

# Get Developing Country Props for these countries:
p_UK2 <- hold4 %>% filter(destination.country == "United Kingdom")
p_UK2 <- readr::parse_integer(p_UK2[[1,2]][1])

p_Ger2 <- hold4 %>% filter(destination.country == "Germany")
p_Ger2 <- readr::parse_integer(p_Ger2[[1,2]][1])

p_SP2 <- hold4 %>% filter(destination.country == "Spain")
p_SP2 <- readr::parse_integer(p_SP2[[1,2]][1])

p_IT2 <- hold4 %>% filter(destination.country == "Italy")
p_IT2 <- readr::parse_integer(p_IT2[[1,2]][1])

p_FR2 <- hold4 %>% filter(destination.country == "France")
p_FR2 <- readr::parse_integer(p_FR2[[1,2]][1])
...
prop.test(x = c(p_UK, p_UK2), n = c(total_DVD, total_DVG)) # Sig
prop.test(x = c(p_Ger, p_Ger2), n = c(total_DVD, total_DVG)) # Sig
prop.test(x = c(p_SP, p_SP2), n = c(total_DVD, total_DVG)) # Sig
prop.test(x = c(p_IT, p_IT2), n = c(total_DVD, total_DVG)) # Sig
prop.test(x = c(p_FR, p_FR2), n = c(total_DVD, total_DVG)) # Sig
# Note on above: x-squared decreases each time.
...
```{r}
# For Developing (Source can Equal Destination):
p_CH <- readr::parse_integer(hold4[[1,2]][1])/total_DVG
p_India <- readr::parse_integer(hold4[[2,2]][1])/total_DVG
p_Indo <- readr::parse_integer(hold4[[3,2]][1])/total_DVG
p_UAE <- readr::parse_integer(hold4[[4,2]][1])/total_DVG
p_Sing <- readr::parse_integer(hold4[[5,2]][1])/total_DVG

# Get Developed COuntry Props for these countries
p_CH2 <- hold3 %>% filter(destination.country == "China")
p_CH2 <- readr::parse_integer(p_CH2[[1,2]][1])

p_India2 <- hold3 %>% filter(destination.country == "India")
p_India2 <- readr::parse_integer(p_India2[[1,2]][1])

p_Indo2 <- hold3 %>% filter(destination.country == "Indonesia")
p_Indo2 <- readr::parse_integer(p_Indo2[[1,2]][1])

p_UAE2 <- hold3 %>% filter(destination.country == "United Arab Emirates")
p_UAE2 <- readr::parse_integer(p_UAE2[[1,2]][1])

p_Sing2 <- hold3 %>% filter(destination.country == "Singapore")
p_Sing2 <- readr::parse_integer(p_Sing2[[1,2]][1])

```

```{r}
# Two-Proportion Z Test:
total <- c(total_DVG, total_DVD)

prop.test(x = c(p_CH, p_CH2), n = total) # Sig 
prop.test(x = c(p_India, p_India2), n = total) # Sig
prop.test(x = c(p_Indo, p_Indo2), n = total) # Sig
prop.test(x = c(p_UAE, p_UAE2), n = total) # Sig
prop.test(x = c(p_Sing, p_Sing2), n = total) # Sig

# Note: all X-squared values lower for these proportion tests
```
