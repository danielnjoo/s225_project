...
Numbers of flights/source country over entire dataset:
...
Orange spike on the right corroborates earlier hypothesis of growth in numbers of flights between year 2010-2011.
...
Huge sd (531.3929) and IQR of 150.25... identifying countries 2sds above the mean (i.e. 1278):
...
Similarly huge  sd (530.3375, difference of 1.05 from outbound flights) and same IQR of 150.25... identifying countries 2sds above the mean (i.e. 1276):
...
Positive linear relationship suggests more GDP growth -> more flights, note this is a count of flights across the entire dataset, and does not therefore suggest a direct increase in the number of flights for countries with high GDP growth.
...
As we don't have date information, we can compare normalized flights for this group of countries vs. undeveloped countries, and see if there's a significant difference.

### Travel Locations

```{r}
data %>% group_by(destination.country, source.country, developed.x) %>% summarize(count=n())  -> temp
temp %>% filter(developed.x==1, destination.country!=source.country) %>% arrange(desc(count)) %>% head(5)
temp %>% filter(developed.x==0, destination.country!=source.country) %>% arrange(desc(count)) %>% head(5)
```

Without normalization, we find that the top 5 overseas destinations of developed countries are Spain, the UK, Germany (x2), and Spain.

Whereas for developing countries, these destinations are Japan, the UAE, China, Guinea, and Singapore.
