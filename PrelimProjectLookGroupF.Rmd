...
Since the p-value is small, at $\alpha = 0.01$, we reject the null hypothesis and conclude that the slope coefficient for `avg_growth.x` is significantly different from $0$. In other words, the model suggests that `avg_growth.x` is a significant predictor of the normalized `log_counts` of flights. The coefficient of $-0.172$ indicates that for every 1 unit increase in the `avg_growth.x`, the noramlized `log_counts` is estimated to decrease by approximately $0.172$. While `avg_growth.x` is a significant predicotr, the model itself has low predictive power. The model only accounts for $4.58$% of the variability in `log_counts` (Multiple $R^{2} = 0.04588$).
...
The t-value for the slope coefficient of `avg_growth.x` is $-.178$ with an associated p-value of $0.00065$. At $\alpha = 0.01$, this is a significant p-value. Although we've already verified the Equal Variance condition above, we plot the residuals directly from the our non-parametric model below:
...
What  we notice here is that all the  relevant countries appear to be low population countries situated between Europe and North  America (the two most  developed continents), and therefore are most likely transitting stops  for longer flights.
...
Our  question of interest here,  therefore, is to  test with statistical significance whether  developing nations do  not fly to popular developed nation destinations.
...
For each location we perform a separate `binom.test()` which takes a vector of success/failure counts and the sample size. In this context, successes are flights to the relevant destination, failures are the complment of this (total-successes), and sample size are total flights.
...
We thus find all our p-values for the nonparametric binomial test, and if we compare them to our p-values from the parametric test:
