...
### Data Read-In:
```{r}
groupfdata <- read_csv("https://awagaman.people.amherst.edu/stat225/datasetsS2018/groupFdata.csv") %>% dplyr::select(-X1)
...
groupfdata %>% group_by(Country.Code.x, developed.x, source.population) %>% 
  summarize(count=n()) -> temp; temp

temp <- temp %>% mutate(count = count/source.population) %>% group_by(Country.Code.x,developed.x) %>% summarize(count = sum(count)); temp # Two very similar values of population for countries? 
...
```{r, warning = FALSE}
groupfdata %>% group_by(source.country, avg_growth.x, source.population, developed.x) %>% 
  summarize(count=n()) -> temp

normal <- temp %>% mutate(count = count/source.population) %>% group_by(source.country, avg_growth.x, developed.x) %>% summarize(count = sum(count))
...
normal %>% ggplot(aes(avg_growth.x, log_count, color = as.factor(developed.x))) + 
  geom_point() + 
  geom_smooth(method = 'lm', se=F) +
  ggtitle("SLR between source country avg GDP growth and Normalized # flights")
```
...
afg <- groupfdata %>% filter(source.country == "Afghanistan")
afg # All the same - some not rounding in the above code?

temp <- temp %>% mutate(count = count/source.population) %>% group_by(source.country, avg_growth.x, developed.x) %>% summarize(count = sum(count)); temp # Two very similar values of population for countries?

temp <- temp %>% mutate(log_count = log(count))

temp %>% ggplot(aes(avg_growth.x, log_count, color = as.factor(developed.x))) + 
  geom_point() + 
  geom_smooth(method = 'lm', se=F) +
  ggtitle("SLR between source country avg GDP growth and Normalized # flights")
```
...
```{r}
mod2 <- lm(log_count ~ avg_growth.x, data = temp)
summary(mod2)

mod3 <- lm(log_count ~ avg_growth.x + developed.x, data = temp)
summary(mod3)
```

Normalizing does not improve the model, but logging the normalized counts does. Logging fixes the conditions. R-squared is miniscule:
...
Just a parallel slopes model? 

```{r}
mod4 <- lm(log_count ~ developed.x, data = temp)
summary(mod4)
```

Growth is maybe significant (see mod2), but developed is. And developed accounts for 26% of the variation.
...
> May have minor normality concern.
...
> avg growth significant now after normalization

```{r}
np_mod2 <- rfit(log_count ~ avg_growth.x, data = temp)
summary(np_mod2, overall = 'drop')
```

> Even more significant than the above as well as the parametric equivalent. R squared obnoxiously low.

```{r}
np_mod3 <- rfit(log_count ~ developed.x, data = temp)
summary(np_mod3, overall = 'drop')
```

> Similar to parametric procedure here.
> Do we want to add in a procedure to see if there is a difference in the average counts (or log counts) for devloped vs. developing countries? 
