...
Positive linear relationship suggests more GDP growth is associated with more flights. **Note** that this is a count of flights across the entire dataset, and does not therefore suggest a direct increase in the number of flights for countries with high GDP growth.
...
> I think we want destination counts not connected to a specific source. Revision below?
...
> If we include travel within one's own country:

```{r}
hold3 <- data %>% filter(developed.x == 1) %>%
  group_by(destination.country) %>%
  summarize(count = n()) %>%
  arrange(desc(count)) %>%
  head(5); hold3
