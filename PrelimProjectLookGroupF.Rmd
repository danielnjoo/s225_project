...
groupfdata <- read.csv("Data/final_data.csv") %>% .[2:ncol(.)] # Fixed Issue.
groupfdata %>% dim()
```
...
temp %>% ggplot(aes(Country.Code.x,count, fill=as.factor(developed.x))) + 
  ...
  coord_flip() + 
  labs(title = "Number of Flights per Source Country", x = "Country", y = "Count", fill="Developed?") + 
  scale_x_discrete(breaks = levels(factor(groupfdata$Country.Code.x))[F])
```

```{r echo=F}
temp %>% arrange(desc(count)) %>% .$Country.Code.x -> topnames
factor(groupfdata$Country.Code.x) %>% unique() -> namelist
match(topnames[1:10], namelist) %>% sort() -> names
...
temp <- temp %>% mutate(count = count/source.population)
