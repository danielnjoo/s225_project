...
The question of interest here, is to test with statistical significance whether  developing nations fly to popular developed nation destinations as frequently as developed nations. A  stacked  bar plot (see Technical Appendix) shows some initial issues, namely that for three of the destinations being considered, Bermuda, Iceland, and St. Kitts and Nevis (KNA) there were no flights originating in  developing  nations whatsoever, this means that the sample size condition for the  proprtion test that $np_0>10$ and $n(1-p_0)>10$ were not met. Additionally, it is not met for Antigua and Barbuda for  which there were only two developing flights. 

Therefore, we report only the p-value for the proportion test for Austria which  was $5.55e-19$ and thus significant at a $0.01$ significance level, allowing us to conclude that developing countries did not fly to Austria at the same rate  as developed countries.

Because these conditions weren't met for the parametric proportion test, a non-parametric binomial test may prove more useful here. When we conducted this on all five popular developed destinations we found the following p-values: $7.73e-02$, $4.72e-30$, $2.747e-01$, $7.46e-08$, and $2.75e-01$, which correspond to Antigua and Barbuda, Austria, Bermuda, Iceland, and Saint Kitts and Nevis respectively. We therefore only found sufficient evidence from the binomial test to reject the null at a $0.01$ significance level for Austria and Iceland. 
