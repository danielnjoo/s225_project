...
Two statistical procedures, parametric ANOVA and Kruskal-Wallis rank sum, for measuring the differences in center were used to determine if there was a significant difference in flights between countries of different developmental status. Both procedures indicated that there is a significant difference between the means and medians of the natural log of flights normalized by the  source country's population. Further, multiple comparison tests (Fishers' LSD and Dunn) give us sufficient evidence to conclude that a significant difference in centers exists between developed and developing countries and developed and recently developed countries,  but not between developing and recently developed countries.
...
We then used  proportion and binomial tests to investigate our question statistically. In our tests, we compared the proportions of flights to the `top5` developed nation destinations to total nation status flight counts, between developing and developed nations. So our $p$ is the proportion of developed nation flights are to Antigua and Barbuda, Iceland, Saint Kitts and Nevis, Bermuda, and Austria, while developing nation flight proportions to the same  destinations are $p_0$. Our hypotheses are thus: $H_0:p_0=p$, and $H_A:p_0\neq p$. 
...
The conclusions drawn from our analysis are limited by our inability to account for the year in which the flight occurred. If a `Year` variable was available, additional regression models could have been fit using each countries GDP per year and flights flown that year. Additional comparisons could have also been made between flights in 2010 for countries of various developmental status to flights in 2016. For our current analysis, year may serve as a confounding variable that we cannot adequately account for. More nuanced and helpful models could have been produced and additional conclusions could have been drawn if this variable was made available.
